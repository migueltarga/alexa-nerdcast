...
var stateHandlers = {
    ...
    playModeIntentHandlers : Alexa.CreateStateHandler(constants.states.PLAY_MODE, {
        ...
        'LaunchRequest' : function () {
            ...
            if (this.attributes['playbackFinished']) {
                ...
                message = 'You were listening to ' + audioData.episodes[this.attributes['playOrder'][this.attributes['index']]].category
                 + ' ' + audioData.episodes[this.attributes['playOrder'][this.attributes['index']]].number +
        ...
        'AMAZON.HelpIntent' : function () {
            ...
            var message = 'You are listening to the Nerd Cast. You can say, Next or Previous to navigate through the playlist. ' +
    ...
    resumeDecisionModeIntentHandlers : Alexa.CreateStateHandler(constants.states.RESUME_DECISION_MODE, {
         ...
         */
        'LaunchRequest' : function () {
            var message = 'You were listening to ' + audioData.episodes[this.attributes['playOrder'][this.attributes['index']]].category
                 + ' ' + audioData.episodes[this.attributes['playOrder'][this.attributes['index']]].number +
                ' Would you like to resume?';
        ...
        'AMAZON.HelpIntent' : function () {
            var message = 'You were listening to ' + audioData.episodes[this.attributes['index']].category +
                + ' ' + audioData.episodes[this.attributes['index']].number +
                ' Would you like to resume?';
            var reprompt = 'You can say yes to resume or no to play from the top.';
            this.response.speak(message).listen(reprompt);
            this.emit(':responseReady');
...
var controller = function () {
    return {
        play: function () {
            ...
            var podcast = audioData.episodes[this.attributes['playOrder'][this.attributes['index']]];
            ...
            if (canThrowCard.call(this)) {
                ...
                var cardContent = 'Playing ' + podcast.description;
                this.response.cardRenderer(cardTitle, cardContent, null);
            }

            var message = "This is " + audioData.episodes[this.attributes['index']].category + ' ' + audioData.episodes[this.attributes['index']].number;
            this.response.speak(message).audioPlayerPlay(playBehavior, podcast.enclosure.url, token, null, offsetInMilliseconds);
            this.emit(':responseReady');
        },
        stop: function () {
            /*
        ...
        playNext: function () {
            ...
            if (index === audioData.episodes.length) {
...
function shuffleOrder(callback) {
    ...
    var array = Array.apply(null, {length: audioData.episodes.length}).map(Number.call, Number);
